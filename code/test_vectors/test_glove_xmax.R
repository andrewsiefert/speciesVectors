...
# read in GloVespecies vectors
paths <- list.files("vectors/xmax", full.names = T) %>%
...
saveRDS(tests, "results/glove_xmax_test_results.rds")


# organize output ---------------------------------------------------------

# read in grid of species vector parameters
grid <- read.csv("code/train_vectors/xmax/grid.csv") %>% as_tibble()
...
out <- map_df(tests, ~data.frame(rmse = mean(.$resample$RMSE),
...
out %>%
  ...
  ggplot(aes(x = xmax, y = r2, color = factor(dim))) +
  geom_point() +
  geom_path() +
...
# save results
write_csv(out, "results/glove_xmax_test_results.csv")
