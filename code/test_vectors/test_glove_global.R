...
null_out <- data.frame(model = "null",
...
trait_out <- data.frame(rmse = mean(enet_trait$resample$RMSE),
                        ...
                        r2 = mean(enet_trait$resample$Rsquared),
                        r2_se = se(enet_trait$resample$Rsquared),
                        mae = mean(enet_trait$resample$MAE),
                        mae_se = se(enet_trait$resample$MAE)) %>%
  mutate(model = "trait")
...
phylo_out <- data.frame(rmse = mean(enet_phylo$resample$RMSE),
                        ...
                        r2_se = se(enet_phylo$resample$Rsquared),
                        mae = mean(enet_phylo$resample$MAE),
                        mae_se = se(enet_phylo$resample$MAE)) %>%
  mutate(model = "phylo")
...
out %>%
  ...
  facet_wrap(~test) +
  geom_hline(yintercept = trait_out$r2, linetype = "dashed", color = "purple") +
  geom_hline(yintercept = null_out$r2, linetype = "dashed")
...
out %>%
  ...
  facet_wrap(~test) +
  geom_hline(yintercept = trait_out$rmse, linetype = "dashed", color = "purple") +
  geom_hline(yintercept = null_out$rmse, linetype = "dashed")
...
out %>%
  ...
  geom_path() +
  facet_wrap(~test) +
  geom_hline(yintercept = trait_out$mae, linetype = "dashed", color = "purple") +
  geom_hline(yintercept = null_out$mae, linetype = "dashed")


# save results
write_csv(out, "results/glove_global_test_results.csv")
